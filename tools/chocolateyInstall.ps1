...
$source = 'http://www.scootersoftware.com/download.php'
$HTML = Invoke-WebRequest $source
$version = (($HTML.ParsedHtml.links|  Where-Object {$_.pathname -like "*BCompare*"} | Select-Object pathname -First 1).pathname).replace('BCompare-','').trimend('.exe')
if ($LCID -in $german)
{
